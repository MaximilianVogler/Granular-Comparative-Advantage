...
DATA2 = zeros(11,num_param);
DATA2(1:2,:) = DATA(1:2,:);
DATA2(3,:) = DATA(22,:);
DATA2(4,:) = DATA(26,:);
DATA2(5,:) = DATA(28,:);
DATA2(6:end,:) = DATA(31:end,:);

% DATA contains 34 moments that were initially computed. DATA2 contains the
% moments we used. In particular, the first row contains alpha_v, the
% second contains alpha_u. The third row contains aggregate mean reversion
% and the fourth row micro-level mean reversion. We used those two values
% for calibration. The next 5 rows contain over-identification moments and
% the last two rows granular contributions.

fname = sprintf('Results/Moments/search_grid%d.csv',S);
TTT = cell2table(num2cell(DATA2));
