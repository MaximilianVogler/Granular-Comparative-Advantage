...
%     
    ...
    if itparam == 1
        ...
        fname = sprintf('Results/Calibrating_Graphs8/calibrated_regdata_BC_%d',S);
        fname2 = sprintf('Results/Calibrating_Graphs8/calibrated_regdata_BC_%d.csv',S);
        save(fname,'DATA1');
        ...
        ffname = sprintf('Results/Calibrating_Graphs8/mean_reversion_BC%d',itparam);
        ...
        set(gca,'xtick',[0.5:1:9.5 10.55])
        set(gca,'xticklabel',[{ -1} round(LPCT0(1:4)'*100)/100 {  0} {  0} round(LPCT0(7:end)'*100)/100 1])
        ffname = sprintf('Results/Calibrating_Graphs8/volatility_BC%d.png',itparam);
        saveas(gcf,ffname)
    end
...
% Save Moments
fname = sprintf('Results/Calibrating_Graphs8/turnover_ANOVA_BC.csv');
...
ylabel('Expected change in export share, $\Delta \Lambda_z^\ast$','interpreter','latex','fontsize',20)
xlabel('Deciles of sectors, by granular $\Gamma_z^\ast$','interpreter','latex','fontsize',20)
ffname = sprintf('Results/Calibrating_Graphs8/combined_mean_reversion_BC.png');
saveas(gcf,ffname)
...
ylabel('Variation in export share, $\mathrm{std}(\Delta \Lambda_z^\ast)$','interpreter','latex','fontsize',20)
xlabel('Deciles of sectors, by granular $\Gamma_z^\ast$','interpreter','latex','fontsize',20)
ffname = sprintf('Results/Calibrating_Graphs8/combined_volatility_BC.png');
saveas(gcf,ffname)
